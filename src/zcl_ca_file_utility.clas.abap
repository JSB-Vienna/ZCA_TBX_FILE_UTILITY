...
*   i n s t a n c e   m e t h o d s
    METHODS:
      ...
      "!
      "! @parameter path             | <p class="shorttext synchronized" lang="en">Complete physical path, may incl. file name</p>
      "! @parameter filter           | <p class="shorttext synchronized" lang="en">Generic filename (may end with *)</p>
      "! @parameter sort             | <p class="shorttext synchronized" lang="en">Sort file list (use const MO_FILE_OPTIONS->LIST_SORTING-*)</p>
      "! @parameter content_type          | <p class="shorttext synchronized" lang="en">Value help type (use const MO_FILE_OPTIONS->VALUE_HELP-*)</p>
      "! @parameter file_list        | <p class="shorttext synchronized" lang="en">Content of directory / file list</p>
      "! @raising   zcx_ca_file_utility | <p class="shorttext synchronized" lang="en">Common exception: File handling errors</p>
      get_file_list
        IMPORTING
...
CLASS zcl_ca_file_utility IMPLEMENTATION.
  ...
  METHOD get_file_list.
    ...
    "-----------------------------------------------------------------*
    mo_file_options->is_list_sorting_valid( sort ).
    mo_file_options->is_content_type_valid( content_type ).

    get_physical_filename_handler( path ).
    DATA(lv_path) = mo_path_hdlr->get_path_name( ).

    CASE mv_location.
      WHEN mo_file_options->location-server.
        file_list = get_file_list_of_as( iv_path    = lv_path
                                            iv_filter  = filter
                                            iv_vh_type = content_type ).

      WHEN mo_file_options->location-pc.
        file_list = get_file_list_of_pc( iv_path    = lv_path
                                            iv_filter  = filter
                                            iv_vh_type = content_type ).
    ENDCASE.
    ...
    CASE sort.
      WHEN mo_file_options->list_sorting-by_date_time_changed.
        SORT file_list BY mtime DESCENDING name ASCENDING.
      WHEN mo_file_options->list_sorting-by_file_name.
  ...
  METHOD get_file_list_of_as.
    ...
    mo_file_options->is_content_type_valid( iv_vh_type ).
