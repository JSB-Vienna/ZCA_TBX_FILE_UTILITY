...
*   i n s t a n c e   m e t h o d s
     METHODS:
       ...
       convert_change_time_stamp
         IMPORTING
           directory_entry TYPE zif_ca_directory_handler=>ty_s_directory_entry
         RETURNING
           VALUE(result)   TYPE zif_ca_directory_handler=>ty_s_directory_entry,
 ...
 CLASS zcl_ca_directory_handler_as IMPLEMENTATION.
   ...
   METHOD finish_reading.
     ...
     "Local data definitions
     DATA:
       _error_no      TYPE ty_s_directory_entry-error_no,
...
*/*--------------------------------------------------------------------*/
   ...
   METHOD get_content_from_location.
     ...
     TRY.
         ...
         DO.
           ...
           IF NOT is_file_or_directory_relevant( content_type    = content_type
   ...
   METHOD initialize_reading.
     ...
     finish_reading( ).      "Just to be sure

     DATA(_path_file) = CONV dirname_al11( path_file ).
     CALL 'C_DIR_READ_START' ID 'DIR'    FIELD _path_file
                             ID 'FILE'   FIELD '*'
...
*/*--------------------------------------------------------------------*/
   ...
   METHOD is_file_or_directory_relevant.
     ...
     IF ( content_type EQ cvc_file_util->content_type-directory AND
          directory_entry-dir_name_lower_case IN filter_name )   OR

        "NE DIRECTORY = BOTH or FILE = a specific file (BOTH = navigation) or multiple files (FILE) are
        "requested by the consumer
        ( content_type NE cvc_file_util->content_type-directory AND
          directory_entry-file_name_lower_case IN filter_name ).
       result = abap_true.
     ENDIF.
   ...
   METHOD read_next_directory_entry.
     ...
     result-directory_name      = path_file.
     result-dir_name_lower_case = to_lower( path_file ).      "for filtering
     result-subrc               = sy-subrc.                   "is evaluated in the calling method

     IF sy-subrc EQ 5.
       "Only FILE_NAME is valid due to internal error.
       CLEAR: result-techn_type,     result-length,           result-owner,
   ...
   METHOD set_flag_is_usable.
     ...
     result = xsdbool( directory_entry-length       NE 0                     OR
                     ...
                     ( directory_entry-content_type EQ techn_type-directory AND
                       directory_entry-file_name    CO '.' )                 OR
